...
# add all pair-wise relations between two nodes
    ...
    def add_relation(self, relation):
        if relation != []:
            ...
            if node1 in self.g.nodes and node2 in self.g.nodes:# self.relation_extractor.rels.index(edge), self.relation_extractor.relational_colors[edge]
                # print(node1, node2)
                self.g.add_edge(node1, node2, value=edge, label=edge, color="black") #relations might need to be turned into objects not just remain strings
